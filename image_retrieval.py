...
class ImageRetriever(object):
    ...
    def __init__(self, path_to_data: str, path_to_all_images: str, bins, func_distance):
    ...
    def image2vector(image_list: np.ndarray, _threshold: list) -> np.ndarray:
        ...
        vector = np.asarray([[np.count_nonzero(m == c) for c in range(num_class)] for m in image_list], dtype=np.float64)
...
def main(flags):
    retriever = ImageRetriever(flags.base_dir, flags.data, bins=flags.bins, func_distance=flags.func_distance)
